...
import { replaceImports } from './utils/replaceImports';
...
export class File {
    ...
    parse() {
        ...
        localImports.forEach((localImport) => {
            ...
            const filePath = path.relative(this.pkg.pkgRoot, resolve.sync(localImport, {
        ...
        depImports.forEach((depImport) => {
            ...
            if (scoped) {
                if (len !== 2) {
                    console.warn(chalk.yellow(`暂不支持 deepImport, ${depImport}`));
